...
name: lead-capture
description: >
  Detect new leads from incoming emails and create them in coWork CRM. Use when
...
Scan for property inquiries and create contacts and opportunities in coWork CRM.
...
2. **Check for duplicates.** Search coWork for existing contacts:
   ```bash
   bash "${CLAUDE_PLUGIN_ROOT}/scripts/cowork-api.sh" GET "/api/v2/contacts?search=email%20%3D%3D%20%22{email}%22&limit=5"
   ```
   Also try by phone:
   ```bash
   bash "${CLAUDE_PLUGIN_ROOT}/scripts/cowork-api.sh" GET "/api/v2/contacts?search=phone%20%3D%3D%20%22{phone}%22&limit=5"
   ```
...
4. **Create in CRM** after user confirmation:
   ...
   New contact:
   ```bash
   bash "${CLAUDE_PLUGIN_ROOT}/scripts/cowork-api.sh" POST "/api/v2/contacts" \
     '{"first_name":"{first}","last_name":"{last}","email":"{email}","phone":"{phone}"}'
   ```

   New opportunity:
   ```bash
   bash "${CLAUDE_PLUGIN_ROOT}/scripts/cowork-api.sh" POST "/api/v2/opportunities" \
...
2. Check for duplicates in coWork
